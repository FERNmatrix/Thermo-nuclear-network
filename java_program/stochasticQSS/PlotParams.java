...
class PlotParams extends Frame implements ItemListener {
    ...
    static PrintWriter toGnuRates;
    static FileOutputStream to25;
    static PrintWriter toGnuRates2;
    
    GraphicsGoodies2 gg = new GraphicsGoodies2();
    
    // Flag for generating gnuplot output file for rate parameters
    static boolean doGnu = false;
    ...
    int plotdim = 60;
    double [][] pp0 = new double[plotdim][5];
    double [][] pp1 = new double[plotdim][5];
    double [][] pp2 = new double[plotdim][5];
    double [][] pp3 = new double[plotdim][5];
    double [][] pp4 = new double[plotdim][5];
    double [][] pp5 = new double[plotdim][5];
    double [][] pp6 = new double[plotdim][5];
    ...
    double [][] rate = new double[plotdim][lpoints];
    double [] temperature = new double [lpoints];
    String [] rreacString = new String[plotdim];
    int [] reaclibClass = new int[plotdim];
    int[] maxcurves = new int[plotdim];
...
/*
  ...
  */
    ...
    public void doRatePlotFrame() {
        ...
        int numberCurves = rl.numberGroups;
        
        // Output to data files
        try{
            String tmpS24 = "output/gnuplotRates.data";
            String tmpS25 = "output/gnuplotRates2.data";
            to24 = new FileOutputStream(tmpS24);
            to25 = new FileOutputStream(tmpS25);
			toGnuRates = new PrintWriter(to24);
			toGnuRates2 = new PrintWriter(to25);
			
			for (int i=0; i<numberCurves; i++) {
                for(int j=0; j<maxcurves[i]; j++){
                toGnuRates.println(rreacString[i]+" "+pp0[i][j]+" "+pp1[i][j]
                    +" "+pp2[i][j]+" "+pp3[i][j]+" "+pp4[i][j]+" "+pp5[i][j]+" "+pp6[i][j]);
                }
            }
            
            toGnuRates2.println("Rate "+numberCurves+" "+lpoints);
            String classes = "";
            for (int i=0; i<numberCurves;i++){
                classes += reaclibClass[i]+" ";
            }
            toGnuRates2.println(classes);
            String header ="logT ";
            for (int i=0; i<numberCurves; i++) {
                header += rreacString[i]+" ";
            }
            toGnuRates2.println(header);
            String dataLine = "";
            for (int i=0; i<lpoints; i++) {
                dataLine = gg.decimalPlace(4,temperature[i])+" ";
                for (int j=0; j<numberCurves; j++){
                    dataLine += gg.decimalPlace(3,rate[j][i])+" ";
                }
                toGnuRates2.println(dataLine);
            }
    ...
    public void rpfInitialize() {
		...
		int framewidth = StochasticElements.widthRatePLotFrame;
		int frameheight = StochasticElements.heightRatePlotFrame;
		
        rpf.pack();
        rpf.setSize(framewidth, frameheight);
    ...
    void loadTotalCurve(int mainIndex, int numberFound) {
        ...
        for (int i=0; i<lpoints; i++) {
            ...
            for (int j=0; j<rl.reactionGroups[numberFound]; j++) {
                ...
                if(i==0 && doGnu){
                    if(j==0){
                        reacString = Cvert.replaceWhiteSpace(rl.rArray[mainIndex+j].reacString,"");
                        reaclibC = rl.rArray[mainIndex+j].reacIndex;
                    }
                    p0 = rl.rArray[mainIndex+j].p0;
                    p1 = rl.rArray[mainIndex+j].p1;
                    p2 = rl.rArray[mainIndex+j].p2;
                    p3 = rl.rArray[mainIndex+j].p3;
                    p4 = rl.rArray[mainIndex+j].p4;
                    p5 = rl.rArray[mainIndex+j].p5;
                    p6 = rl.rArray[mainIndex+j].p6;
                    
                    pp0[numberFound][j] = p0;
                    pp1[numberFound][j] = p1;
                    pp2[numberFound][j] = p2;
                    pp3[numberFound][j] = p3;
                    pp4[numberFound][j] = p4;
                    pp5[numberFound][j] = p5;
                    pp6[numberFound][j] = p6;
                    maxcurves[numberFound] = rl.reactionGroups[numberFound];
                }
            ...
            if(i==0 && doGnu){
                ...
                // Store reaction string with whitespace removed and --> converted to ->
                rreacString[numberFound] = Cvert.replaceWhiteSpace(reacString,"");
                rreacString[numberFound] = StochasticElements.replaceThisWithThat(rreacString[numberFound],"--","-");
                reaclibClass[numberFound] = reaclibC;
            }
            ...
            rate[numberFound][i] = temp;//Math.log10(temp);
            temperature[i] = T9*1e9;//Math.log10(T9*1e9);
        }
