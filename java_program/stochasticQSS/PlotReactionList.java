...
class PlotReactionList extends Panel {
	...
	*/
    ...
    int maxCases = 151;     // Max number of reaction entries
...
//                             System.out.println("PlotReactionList: m="+m+" mm="+mm+" file="+file
//                                 +" length="+numberObjects +" label="+tryIt.reacString
//                                 +" RnotActive="+DataHolder.RnotActive[Z][N][m]+" mm="+mm+" no="+numberObjects);
                            ...
                            // Here rather than below to keep continue statements from causing eof read error
                            mm++;   

                            // Store in plotting array if valid reaction (reaction involves isotope with this
                            // Z and N, and all species on both sides of the reaction are in the network.
                            
                            if( !DataHolder.RnotActive[Z][N][m] ) {
                            
                                // Omit light-ion reactions that don't include other species in network
                                ...
                                if(Z==1 && N==1 && !StochasticElements.isLightIonReaction(tryIt)) continue;
                                if(Z==2 && N==2 && !StochasticElements.isLightIonReaction(tryIt)) continue;
                                
                                // Add reaction to plotting array only if the reaction stays in the network
                                
                                if(StochasticElements.pruneReactions(Z,N,tryIt)){
                                    ...
                                    if(m < maxCases -1) { m++; }
                                        ...
                                        // Note that the final argument for makeTheWarning below must be a
                                        // reference to a Frame.  The present object is not a Frame, so we 
                                        // reference the Frame in which it sits (The instance
